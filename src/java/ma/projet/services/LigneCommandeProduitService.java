...
public class LigneCommandeProduitService implements IDao<LigneCommandeProduit> {
    ...
    public LigneCommandeProduit getById(int produitId, int commandeId) {
        LigneCommandeProduit ligneCommandeProduit = null;
        Session session = null;
        Transaction tx = null;
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            tx = session.beginTransaction();
            String hql = "FROM LigneCommandeProduit lcp WHERE lcp.id.produit = :produitId AND lcp.id.commande = :commandeId";
